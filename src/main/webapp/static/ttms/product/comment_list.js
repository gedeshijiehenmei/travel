...
function doBtnDel(){
	 var id=$(this).parents("tr").find("td:eq(0)").text();
		...
		if(confirm("您确定要删除编号为【"+id+"】的评论吗？")){
			$.ajax({
				url:"deleteCommentInfoByid/"+this.id,
				type:"DELETE",
...
/*	if($(this).hasClass("btn-add")){
	...
	if($(this).hasClass("btn-update")){
		...
		//data(key)函数用于获取数据
		$("#modal-dialog").data("pid",$(this).parent().parent().data("pid"));
		//定义修改时的标题
		title="修改订单编号为"+$("#modal-dialog").data("pid")+"订单信息";
	}
	//2.启动模态框,并加载页面
	//在模态框对应位置异步加载url
	var url="commentInfo_edit";
...
function getQueryFormData(){
	...
	var params={
		scenicSpotName:$("#searchNameId").val(),
...
function setTableBodyRows(result){//result 为一个json对象
	...
	var tr=$("<tr></tr>");
	//在tr对象上绑定一个值
	tr.data("pid",result[i].pid);//key/value
	//tr对象中追加td字符串对象
	/*var firstTd='<td><input type="checkbox" name="checkedItem" value="[pid]"></td>';
	//将firstTd字符串中的[id]替换为一个具体指
	firstTd=firstTd.replace("[pid]",result[i].pid);
	tr.append(firstTd);	*/
	tr.append("<td>"+result[i].pid+"</td>");
	tr.append("<td>"+result[i].userid+"</td>");
	tr.append("<td>"+result[i].dindanid+"</td>");
	tr.append("<td>"+result[i].scenicSpotName.slice(0,10)+"...</td>");
	tr.append("<td>"+result[i].fraction+"</td>");
	tr.append("<td>"+result[i].content.slice(0,15)+"...</td>");	
	tr.append("<td>"+result[i].commentaryTime+"</td>");
	/*tr.append("<td><button type='button' class='btn btn-default btn-update'>修改</button></td>");*/
	tr.append("<td><button type='button' id="+result[i].pid+" class='btn btn-default btn-del'>删除</button></td>");
