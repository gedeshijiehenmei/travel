...
function doFindObjectById(id){
	var url="doFindDindanInfoById"
	var params={"id":id};
	$.post(url,params,function(result){
		if(result.code==100){
		 doInitEditFormData(result.extend.dindanInfo);	
		}else{
...
function doInitEditFormData(obj){
	...
	$("#contactsId").val(obj.contacts);
	$("#phoneId").val(obj.phone);
	$("#departureTimeId").val(obj.departureTime);
}
...
function doSaveOrUpdate(){
	...
	//根据id是否有值来判定是修改还是添加
	var updateUrl="doUpdateDindanInfo";
	var insertUrl="doSaveDindanInfo";
...
function getEditFormData(){
 //1.获得页面上用户输入的数据,封装为json对象(相对比较灵活)
  var params={//根据id获得数据
	  ...
	  "contacts":$("#contactsId").val(),
	  "phone":$("#phoneId").val(),
	  "departureTime":$("#departureTimeId").val(),
  }
