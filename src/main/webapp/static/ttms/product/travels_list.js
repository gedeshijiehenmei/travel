...
function doBtnDel(){
	 var title=$(this).parents("tr").find("td:eq(4)").text();
		if(confirm("您确定要删除【"+title+"】吗？")){
			$.ajax({
				url:"deltravelsInfoByid/"+this.id,
				type:"DELETE",
...
function setTableBodyRows(result){//result 为一个json对象
	...
	tr.append("<td>"+'<img src="/image/'+result[i].cover+'" class="image" />'+"</td>");
	tr.append("<td>"+result[i].title.slice(0,12)+"...</td>");
	tr.append("<td>"+result[i].introduction.slice(0,12)+"...</td>");
	tr.append("<td>正文内容较多请前往客户端查看</td>");
