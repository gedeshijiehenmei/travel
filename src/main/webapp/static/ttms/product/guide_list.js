...
function doBtnDel(){
	 var userName=$(this).parents("tr").find("td:eq(2)").text();
		...
		if(confirm("您确定要删除【"+userName+"】吗？")){
			$.ajax({
				url:"deleteuserInfoByid/"+this.id,
				type:"DELETE",
...
function doShowEditDialog(){
	...
	if($(this).hasClass("btn-update")){
		...
		//定义修改时的标题
		title="修改id为"+$("#modal-dialog").data("id")+"导游信息";
	}
	//2.启动模态框,并加载页面
	//在模态框对应位置异步加载url
	var url="guideInfo_edit";
...
function setTableBodyRows(result){//result 为一个json对象
	...
	tr.append("<td>"+result[i].password+"</td>");
	...
	/*tr.append("<td>"+result[i].photo+"</td>");*/
	tr.append("<td>"+'<img src="/image/'+result[i].photo+'" class="image" />'+"</td>");
	tr.append("<td>"+result[i].phone+"</td>");
	tr.append("<td>"+result[i].mailbox+"</td>");
	tr.append("<td>"+(result[i].state==1?"启用":"禁用")+"</td>");
